...
from .pytorch_common_inputs import (PytorchComponentBaseInputs,
                                   PytorchComponentBaseOutputs,
                                   PytorchComponentInputValidator,
                                   PytorchComponentOutputValidator,
                                   PytorchIOValue)

IT = TypeVar("IT", bound=PytorchComponentBaseInputs)  # Input Type
OT = TypeVar("OT", bound=PytorchComponentBaseOutputs)  # Output Type


class PytorchComponentSpec(Generic[IT, OT]):
    ...
    INPUTS: IT = PytorchComponentBaseInputs()
    OUTPUTS: OT = PytorchComponentBaseOutputs()
    ...
    def __init__(
        ...
        if self.OUTPUTS:
            parsed_key_to_output_key = {
                f"{output_key}{PytorchComponentSpec.OUTPUT_ARGUMENT_SUFFIX}": output_key
    ...
    def _validate_spec(cls):
        ...
        if cls.INPUTS:
            for key, val in cls.INPUTS.__dict__.items():
                if not isinstance(val, PytorchComponentInputValidator):
                    raise ValueError(
                        f"Input {key} is not of type {PytorchComponentInputValidator.__name__}"
                    )

        if cls.OUTPUTS:
            for key, val in cls.OUTPUTS.__dict__.items():
                if not isinstance(val, PytorchComponentOutputValidator):
                    raise ValueError(
                        f"Output {key} is not of type {PytorchComponentOutputValidator.__name__}"
                    )
        pass
    ...
    def _parser(self):
        ...
        if self.OUTPUTS:
            for key, props in self.OUTPUTS.__dict__.items():
                ...
                parser.add_argument(
                    f"--{key}{PytorchComponentSpec.OUTPUT_ARGUMENT_SUFFIX}",
                    default=f"/tmp/{key}",
                    type=str,
